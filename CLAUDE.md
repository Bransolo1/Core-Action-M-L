...
| State / Persistence | React Context + localStorage + DB sync via `/api/sync` |
...
│   ├── dashboard/page.tsx     # KPIs, Weeks of Cover, Reorder Alerts
...
│   ├── abc-analysis/page.tsx  # ABC / Pareto product ranking
│   ├── import/page.tsx        # CSV import (Products, Sales, Shopify, Woo)
│   ├── suppliers/page.tsx     # Supplier management
│   ├── login/page.tsx         # NextAuth login
│   ├── settings/page.tsx      # Order cycles, seasonal factors, OTB
│   └── api/
│       ├── sync/route.ts      # GET/POST — full state hydrate/persist
...
│       └── template/          # GET — CSV template downloads
│
├── components/
│   ├── layout/
│   │   ├── Header.tsx
...
```bash
./start.sh           # One-click: installs deps, creates DB, seeds, launches
# OR manually:
npm install
...
npm run test         # Vitest unit tests (73 tests)
...
|----------|----------|-------------|
| `DATABASE_URL` | Yes | `file:./data/coreaction.db` (auto-set by start.sh) |
| `NEXTAUTH_SECRET` | Yes | Random 32-byte base64 (auto-generated by start.sh) |
| `NEXTAUTH_URL` | Yes | `http://localhost:3000` |
| `ANTHROPIC_API_KEY` | No | Claude API key for AI suggestions (optional) |
